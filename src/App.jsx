...
function App() {
  ...
  const getDynamicBg = (temp) => {
    if (temp >= 25) return "linear-gradient(135deg, rgba(250, 54, 0, 0.7) 0%, rgba(255, 100, 0, 0.5) 100%)";
    if (temp >= 20) return "linear-gradient(135deg, rgba(255, 215, 0, 0.7) 0%, rgba(255, 165, 0, 0.5) 100%)";
    if (temp >= 15) return "linear-gradient(135deg, rgba(100, 230, 100, 0.7) 0%, rgba(34, 139, 34, 0.5) 100%)";
    ...
    <div className="main-wrapper">
      ...
      <div className="input-group">
        <input
          ...
          onKeyDown={(e) => { if (e.key === 'Enter') fetchOpenWeatherApi(); }}
        />
        ...
        {weatherData?.main && (
          <div className="weather-current">
            <div className='tile' style={{ background: getDynamicBg(weatherData.main.temp) }}>
              <img
                src={weatherData.dt > weatherData.sys.sunrise && weatherData.dt < weatherData.sys.sunset
                    ? (weatherIcons[weatherData.weather[0].main] || icons.clouds)
                    : icons.moon}
                alt="Wetter"
              />
        ...
        {forecastData?.list && (
          <div className="forecast-grid">
            {forecastData.list.filter((_, index) => index % 8 === 0).map((item, index) => {
              ...
              return (
                <div key={index} className='tile' style={{ background: getDynamicBg(temp) }}>
